...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const populatePageContent = () => {
        ...
        const creator = settings.apiSettings?.creator || 'RijalGanzz';

        setPageContent(DOM.pageTitle, settings.name, "Rijal API");
        setPageContent(DOM.wm, `© ${currentYear} ${creator}. Semua hak dilindungi.`);
        setPageContent(DOM.appName, settings.name, "Rijal API");
        ...
        if (DOM.apiLinksContainer) {
            ...
            const defaultLinks = [{ url: "https://chat.whatsapp.com/IxlDjH5B5VBFR6nqjhcE0M", name: "Lihat di group", icon: "fab fa-group" }];
